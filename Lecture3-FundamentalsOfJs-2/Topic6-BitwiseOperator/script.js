...
// IMPORTANT NOTE: When performing right shift operations on negative numbers, the sign bit (leftmost bit) is preserved and copied into the positions that are vacated during the shift. This means when we shift a bit of negative number towards right then the result remains negative. This is called arithmetic right shift, which preserves the sign of the number.it does that  because of the mathematical calculations involved in this operation:
...
// because of the fact that zeros are filled from the left:This is why we get a large positive number (2147483644) instead of a negative number:because the negative-number are stored in form of two's complement format:that's why we get the large positive number:
