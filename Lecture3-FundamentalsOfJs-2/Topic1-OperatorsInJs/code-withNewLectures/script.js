...
// -> IMP-NOTE: Augmented assignment operators are used to assign a value to a variable after performing some operation or operation which we have provided to it: on that variable:
// -> addition assignment:
let d = 5;
d += 2;
console.log(d);
...
// -> that's why if we directly use this operator type in console-statement:To increase our value:then we are not gonna be able to do that:because at that time our value is not gets (increased):so it gonna be print the same value as our variable have:
let y = 1;
